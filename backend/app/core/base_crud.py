...
from sqlalchemy import asc, delete, desc, false, func, select, update
...
class CRUDBase[ModelType: MappedBase, CreateSchemaType: BaseModel, UpdateSchemaType: BaseModel]:
    ...
    async def get(self, preload: list[str | Any] | None = None, **kwargs) -> ModelType | None:
        ...
        try:
            ...
            raise HTTPException(detail=f"获取查询失败: {e!s}", status_code=500)
    ...
    async def list(
        ...
        try:
            ...
            raise HTTPException(detail=f"列表查询失败: {e!s}", status_code=500)
    ...
    async def tree_list(
        ...
        try:
            ...
            raise HTTPException(detail=f"树形列表查询失败: {e!s}", status_code=500)

    async def page[OutSchemaType: BaseModel](
        self,
        offset: int,
        limit: int,
        ...
        try:
            ...
            raise HTTPException(detail=f"分页查询失败: {e!s}", status_code=500)
    ...
    async def create(self, data: CreateSchemaType | dict) -> ModelType:
        ...
        try:
            ...
            raise HTTPException(detail=f"创建失败: {e!s}", status_code=500)
    ...
    async def update(self, id: int, data: UpdateSchemaType | dict) -> ModelType:
        ...
        try:
            ...
            if not obj:
                raise HTTPException(detail="更新对象不存在", status_code=400)
            ...
            if not verify_obj:
                # 对象已被删除或权限已失效
                raise HTTPException(detail="更新失败，对象不存在或无权限访问", status_code=403)

            return obj
        except Exception as e:
            print(e)
            raise HTTPException(detail=f"更新失败: {e!s}", status_code=500)
    ...
    async def delete(self, ids: builtins.list[int]) -> None:
        ...
        try:
            ...
            pk_cols = list(getattr(mapper, "primary_key", []))
            if not pk_cols:
                raise HTTPException(detail="模型缺少主键，无法删除", status_code=400)
            if len(pk_cols) > 1:
                raise HTTPException(detail="暂不支持复合主键的批量删除", status_code=400)
            ...
            raise HTTPException(detail=f"删除失败: {e!s}", status_code=500)
    ...
    async def clear(self) -> None:
        ...
        try:
            ...
            raise HTTPException(detail=f"清空失败: {e!s}", status_code=500)
    ...
    async def set(self, ids: builtins.list[int], **kwargs) -> None:
        ...
        try:
            ...
            pk_cols = list(getattr(mapper, "primary_key", []))
            if not pk_cols:
                raise HTTPException(detail="模型缺少主键，无法更新", status_code=400)
            if len(pk_cols) > 1:
                raise HTTPException(detail="暂不支持复合主键的批量更新", status_code=400)
            ...
            raise HTTPException(detail=f"批量更新失败: {e!s}", status_code=500)
